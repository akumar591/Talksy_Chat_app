    ...
    <div className="flex flex-col w-full h-full bg-[var(--bg)] text-[var(--text)]">
          ...
          <div ref={menuRef} className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-52 glass rounded-xl p-2 z-50">
                <div   onClick={() => {navigate(`/user/${chat.id}`); setShowMenu(false);}}
                       className="px-3 py-2 hover:bg-[var(--primary)]/10 rounded flex gap-2  cursor-pointer">
                  <FiUser /> Profile
                </div>
                <div
                  onClick={() => navigate(`/media/${chat.id}`)}
                  className="px-3 py-2 hover:bg-[var(--primary)]/10 rounded flex gap-2 cursor-pointer"
                >
                  <FiImage /> Media
                </div>
                ...
                <div
                  onClick={() => setMessages([])}
                  className="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg cursor-pointer text-red-400 hover:bg-red-500/10 transition"
                >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {messages.map((msg) => (
          ...
          >
            <div
              className={`relative px-4 py-2 rounded-2xl max-w-[70%] group
        ${
          msg.sender === "me"
            ...
            >
              ...
              <div
                ...
                } hidden group-hover:flex gap-2 glass px-2 py-1 rounded-lg shadow z-40`}
              >
                <button
                ...
                <button
                  ...
                  className="flex items-center justify-center w-7 h-7 rounded-full hover:bg-[var(--primary)]/20 transition text-[var(--text)]"
                >
                  <FiSmile />
                </button>
              </div>

              {/* 🔥 FLOATING REACTION PICKER FIXED */}
              {reactionMsgId === msg.id && (
                <div
                  className={`absolute -bottom-16 ${
                    msg.sender === "me" ? "right-0" : "left-0"
                  } glass px-2 py-1 rounded-lg flex gap-1 shadow z-50`}
                >
                  {emojis.map((e, i) => (
                    <span
                      key={i}
      ...
      <div className="p-3 bg-[var(--card)] sticky bottom-0">
        ...
        {showEmoji && (
          <div
            ref={emojiRef}
            className="absolute bottom-16 left-10 w-56 glass p-2 rounded-xl flex flex-wrap gap-2 z-50"
          >
            {emojis.map((e, i) => (
              <span
                key={i}
        ...
        {showAttach && (
          <div
            ref={attachRef}
            className="absolute bottom-16 left-3 glass p-2 rounded-xl z-50"
          >
            <div className="px-3 py-2 hover:bg-[var(--primary)]/10 rounded">
              📷 Photo
            </div>
            <div className="px-3 py-2 hover:bg-[var(--primary)]/10 rounded">
              📄 Document
        ...
        <div className="flex items-center gap-2">
          ...
          {input ? (
            <button
              onClick={sendMessage}
              className="w-10 h-10 rounded-full bg-[var(--primary)] flex items-center justify-center text-black"
            >
              <FiSend />
            </button>
          ) : (
